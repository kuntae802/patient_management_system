...
|---|---|---|---|
| `id` | uuid | PK `default gen_random_uuid()` | |
| `actor_id` | uuid | `REFERENCES auth.users(id) ON DELETE SET NULL` nullable | **`users`가 아니라 `auth.users`** — 직원·환자·NULL(시스템) 모두 수용 [§결정 D-6] |
...
- **D-6 (감사 actor FK):** brainstorm은 `audit_logs.user_id FK→users`였으나, 환자도 actor가 될 수 있어(예약 취소·본인정보 reveal) `users` FK는 환자 auth uid를 거부한다. 따라서 `actor_id uuid REFERENCES auth.users(id) ON DELETE SET NULL`로 설계(직원·환자·시스템 모두 수용). [glossary.md L17·L19; architecture.md L187]
