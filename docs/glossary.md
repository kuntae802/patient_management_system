...
| `payment` | 수납 | 헤더 — `payments`(0045·7.1)·내원 1:1(encounter_id UNIQUE)·status draft/finalized/cancelled·billing_type postpaid/prepaid·금액 집계 컬럼은 7.2 집계(`build_payment` 0046·total/covered/non_covered)/7.3 산정/7.4 결제(`finalize_payment` 0048·payment_method·payment_no·finalized_at/by·paid)가 채움. **선후수납=7.8**(`prepay_payment` 0051·선결제 `paid_amount_krw` 단일 누계·billing_type→prepaid 전환·finalize `paid=greatest(copay,paid)` 차액 정산). **취소·환급=7.9**(`settle_cancelled_visit` 0052·cancel_encounter+void+환급 1 txn·`refunded_amount_krw` 선납 전액 환급·`payments_cancelled_consistency` CHECK 취소→cancelled_at 필수·노쇼료 미부과=메커니즘 예비). **문서 출력=7.5/7.6**(진료비 계산서·영수증 `0049`·**세부산정내역서 7.6**·브라우저 인쇄·masked RRN·내보내기 감사 `log_payment_document_export`·document_type=receipt/statement) |
...
| `cancel_encounter(uuid, text)` | RPC(SECURITY DEFINER) | `scheduled\|registered→cancelled`. 권한 `encounter.cancel`(**reception 7.9 grant** — settle 가 호출), `cancelled_at`+`cancel_reason`. 수가 미발생 정산 Epic7 FR-118. **7.9 `settle_cancelled_visit` 가 perform 호출(취소·void·환급 1 txn)** |
...
| `perform_examination(uuid)` / `complete_examination(uuid)` / `perform_treatment_order(uuid)` | RPC(SECURITY DEFINER) | 전진 전이 — 권한 자가 게이트(`examination.perform`/`examination.complete`/`treatment.perform`) + `for update` + 소스상태 precondition(재수행 차단 FR-093). 액터=`auth.uid()`. not-found `PT404`. 0010 `start_consult` 동형. **perform_* 는 7.10 재정의로 `assert_encounter_orderable` 게이트(종결 내원 수행 차단 PT409·0053)** — `complete_examination`(판독)은 게이트 없음(종결 후 정당·fee 0) |
...
| `finalize_payment` | 함수(0048·**0051 재정의 7.8**·SECURITY DEFINER) | draft 수납 건을 finalized 로 전이 + 내원 완료: 헤더 `for update` 룩업 → status≠draft PT409(이중결제 차단)·total≤0 PT409(빈 내원) → 결제 컬럼 기록(status/payment_method/payment_no 인라인 생성/finalized_at/finalized_by=app.actor_id GUC/**paid=`greatest(copay, paid)`** — 7.8 차액 정산·후수납은 `greatest(copay,0)=copay` 무회귀·과납은 선납 보존) → `perform complete_encounter`(in_progress→completed·주상병 게이트 PT422·전이 트리거 재사용). `revoke all from public,anon,authenticated`+`grant execute to service_role`(finalize 위조 차단) |
...
| `DocumentExportRequest` | 요청 스키마(billing.py) | `document_type: Literal['receipt','statement']='receipt'`(7.5 receipt·**7.6 statement 가동**·동일 엔드포인트/RPC·after_data 로 구분) |
| `GET /v1/encounters/{id}/payment/receipt` | 엔드포인트(billing.py) | 문서 데이터(finalized 만·게이트 payment.read). 비-finalized 409 invalid_transition·빌드 전 404·권한 403. **영수증·세부산정내역서 공용 데이터**(라인 전 컬럼·진료기간)·문서별 렌더는 web |
| `POST /v1/encounters/{id}/payment/receipt/export` | 엔드포인트(billing.py·204) | 문서 인쇄/내보내기 감사 기록(`log_payment_document_export` 호출·게이트 payment.read). body=DocumentExportRequest(receipt/statement). payment 미존재 404·권한 403 |
...
| `prepay_payment` | 함수(0051·SECURITY DEFINER·Story 7.8) | 선결제(선수납) — 헤더 upsert(registered 수가 0 이어도)→`for update`→status≠draft PT409(비가역 후 누적 차단)·amount≤0 PT409→`billing_type='prepaid'`+`paid_amount_krw += amount`+`payment_method` 기록(**status 불변=draft**·내원 상태 전이 없음). 감사=`trg_payments_audit` 자동. `revoke all from public,anon,authenticated`+`grant execute to service_role`. 권한=FastAPI `_require_payment_manage`(동일 txn·build/finalize 동형) |
| `PaymentPrepayRequest` | 요청 스키마(billing.py·Story 7.8) | `amount_krw: int Field(gt=0)`(선결제액·Pydantic 1차 422·DB 최종선)·`payment_method: Literal['card','cash','transfer']`. 차액 정산은 finalize(별도 금액 입력 없음) |
| `POST /v1/encounters/{id}/payment/prepay` | 엔드포인트(billing.py·Story 7.8) | 선결제 — `build→price→prepay_payment` 원자 호출(NFR-041). body=PaymentPrepayRequest·게이트 payment.manage. 액션 엔드포인트(내원 상태 전이 없음·완료는 finalize). 이미 결제/취소 또는 금액≤0 409·미존재 404·권한 403 |
...
## 수납 정책 — 후/선수납 (Story 7.8, `0051_payment_prepay.sql`)

> ⚠️ **마이그 번호 0051**: Epic 7 블록 0045~0059(0050 다음). **선후수납 레이어**(0045 컬럼 선언 → … → 0048 결제·완료 → 0051 선후수납). 7.1이 선언만 한 `billing_type`(postpaid/prepaid)·`paid_amount_krw`를 가동. 7.4가 명시 이월한 "선/부분수납=7.8" 청산. **설계 결정(사용자 확정 2026-06-24·AskUserQuestion 4건)**: ① 선수납 진입점=**수납 워크리스트 확장**(`registered`+`in_progress` 노출·상태 칩·동일 billing-detail 화면) ② 정책 범위=**건별 운용**(`billing_type` per payment·후수납 기본 default postpaid·신규 설정 테이블 0) ③ 차액 모델=**단일 누계 `paid_amount_krw`**(선결제 누적·finalize `greatest(copay,paid)`·차액=`copay-paid` 파생·`due_amount_krw`(7.5) 재사용·신규 컬럼 0) ④ 과납(선납>copay)=**표시·플래그 + 환급 7.9 이월**(finalize 허용·초과분 보존).

> **선후수납 경계(Story 7.8 확정):** **신규 마이그 1건(0051)·신규 컬럼 0·신규 권한 0·신규 테이블 0·신규 파일 0**(전부 기존 billing 모듈·RPC·화면 확장). 핵심: `prepay_payment` RPC 신규(선결제 누적·billing_type prepaid 전환·status draft 유지·내원 상태 전이 없음) + `finalize_payment` **1줄 재정의**(`paid=greatest(copay,paid)` — 후수납 무회귀·차액 수금·과납 보존). API: `PaymentPrepayRequest`·`prepay_payment`(service/db)·`POST .../payment/prepay`·`fetch_billing_worklist` registered 포함(`status in ('registered','in_progress')`). Web: 부분(◐·앰버) 결제상태 배지·선수납 패널(금액 입력+선결제 버튼+신원 confirm)·finalize 차액 표시·워크리스트 상태 칩(접수=선수납 가능/진찰중=정산 대상). **선결제 시점**: `registered`(접수 후·진찰 전·수가 0=예치)~`in_progress`(진료 중). finalize=in_progress(complete_encounter 게이트). **단순화/이월**: 거래별 결제수단 원장(헤더 method=최종 정산 method·선결제 method는 감사만)·실제 환급 트랜잭션/취소·노쇼 환급=7.9·10원 단위 강제 미적용. **미구현(후속)**: 취소·노쇼 정산=7.9·부분수행=7.10·환자 포털 수납=8.3.

## 취소·노쇼 정산 — 수가 미발생·선납 환급 (Story 7.9, `0052_payment_cancel.sql`)

> ⚠️ **마이그 번호 0052**: Epic 7 블록 0045~0059(0051 다음). **취소·환급 레이어**(0045 컬럼 선언 → … → 0048 결제·완료 → 0051 선후수납 → 0052 취소·환급). 7.4가 명시 이월한 "취소·노쇼 정산·cancelled 일관성 CHECK=7.9", 7.8이 이월한 "과납·선납 후 0-수가 환급=7.9" 청산. **설계 결정(사용자 확정 2026-06-24·AskUserQuestion 4건)**: ① 정산 진입점=**수납 화면 통합 액션**(billing-detail "내원 취소·환급" → `settle_cancelled_visit` RPC·결제 도메인 수납 일원화) ② 환급 기록=**신규 컬럼 `refunded_amount_krw`**(paid 총 수령 보존·refunded 별도·순납부=paid−refunded·전액 환급) ③ 노쇼료=**정책 OFF 기본·메커니즘 예비만**(`payment_details.fee_item_id` nullable 0045·실제 부과 UI/RPC 미구현) ④ 환급 확인=**신원 confirm(web) + 원결제수단 자동**(`payment_method`)·환급 문서 미출력.

| 용어 | 종류 | 정의 / 위치 |
|---|---|---|
| `refunded_amount_krw` | 컬럼(0052·payments) | 선납 환급액(int≥0·default 0). `paid_amount_krw`(총 수령)는 보존·`refunded`는 반환액 별도 추적 → 순납부=paid−refunded. **`payments_refund_le_paid` CHECK**(refunded≤paid·받은 것보다 더 환급 불가) |
| `payments_cancelled_consistency` | CHECK(0052·0048 이월 청산) | 취소 행 일관성 — `status<>'cancelled' OR cancelled_at is not null`(cancelled 행=취소시각 필수). draft/finalized·기존 cancelled 0 → ADD 무회귀 |
| `settle_cancelled_visit(uuid, text)` | 함수(0052·SECURITY DEFINER·Story 7.9) | 취소·노쇼 정산 — `perform cancel_encounter`(registered→cancelled·상태머신 재사용·encounter.cancel 평가) → draft 수납 `for update` → status≠draft PT409(방어) → void(`status='cancelled'`·`cancelled_at`·`cancel_reason`)+**선납 전액 환급**(`refunded_amount_krw=paid_amount_krw`). 후수납(paid=0)은 refunded=0. 감사=`trg_payments_audit`·encounters 감사 자동. `revoke all from public,anon,authenticated`+`grant execute to service_role`. 권한=FastAPI `_require_payment_manage`(payment.manage) + cancel_encounter 내부 `encounter.cancel`(둘 다 통과·prepay/finalize 동형) |
| `PaymentCancelRequest` | 요청 스키마(billing.py·Story 7.9) | `reason: str\|None Field(max_length=200)`(취소 사유·저민감 운영 텍스트·임상/PII 자유텍스트 금지·`encounters.cancel_reason` 정합·선택) |
| `POST /v1/encounters/{id}/payment/cancel` | 엔드포인트(billing.py·Story 7.9) | 내원 취소·정산 — `build_payment`(draft 보장)→`settle_cancelled_visit`(cancel+void+refund) 원자 호출(NFR-041). body=PaymentCancelRequest·게이트 payment.manage. 액션 엔드포인트(status PATCH 아님). 비-registered/scheduled 또는 비-draft 409·미존재 404·권한(payment.manage·encounter.cancel) 403 |

> **취소·노쇼 정산 경계(Story 7.9 확정):** **신규 마이그 1건(0052)·신규 컬럼 1개(`refunded_amount_krw`)·신규 권한 0·신규 테이블 0·신규 파일 0**(seed reception→`encounter.cancel` grant·0010 기존 권한). 핵심 통찰: **"수가 미발생"은 대부분 구조적** — 진찰료(`registered→in_progress` 트리거)·검사처치료(perform) 적재는 in_progress 진입 후 → 취소·노쇼(진찰 전)는 `fee_items` 0(별도 미발생 처리 로직 없음). 7.9 능동 작업=draft void(특히 선납 prepaid 헤더) + **선납 환급**(`settle_cancelled_visit`) + `payments_cancelled_consistency` CHECK. API: `PaymentCancelRequest`·`settle_cancelled_visit`(service/db·build 선행)·`POST .../payment/cancel`. Web: billing-detail "내원 취소·환급" 버튼(total=0=registered proxy·진찰 전만·in_progress는 완료만)+취소 confirm(환급액·원결제수단)+취소 상태 패널(환급액·사유·취소시각). cancelled 결제상태 배지=기존 `PaymentStatusBadge`(✕·취소선·무변경). **노쇼는 거의 예약 도메인**(0036: `encounters.no_show` 미카운트·예약 미방문→내원 미생성)·`mark_no_show` 방어적. 실무 핵심=registered+선납 취소→환급. **단순화/이월**: 노쇼료 실제 부과 UI/RPC=정책 OFF 기본(스키마 예비만)·부분 환급(취소=전액)·환급 확인증/영수증=미출력(법정 서식 환급 개념 없음)·환급수단 명시 선택·환급 원장 ledger 미채택. **미구현(후속)**: 부분수행 정산=7.10·환자 포털 수납=8.3.

## 부분 수행 정산 · 오더-by-내원상태 게이트 (Story 7.10, `0053_order_encounter_gate.sql`)

> ⚠️ **마이그 번호 0053**: Epic 7 블록 0045~0059(0052 다음). **오더-by-내원상태 게이트 레이어** — Epic 7 마지막 구현 스토리, 이월 갭 ⑥(취소·노쇼·부분수행 정산 경로)을 부분수행으로 완결. **설계 결정(사용자 확정 2026-06-25·AskUserQuestion 3건)**: ① 오더-by-내원상태 게이트=**7.10에서 닫는다**(종결 내원 오더 생성·수행 차단·deferred 5건 청산) ② 미수행 오더 처분=**그대로 둔다**(ordered 잔존·청구는 `fee_items` 부재로 구조적 제외·오더 cancel RPC 추가 안 함) ③ 부분수행 가시성=**카운트 배지**(`pending_orders_count`·payment.read 경로·order.read grant 0).

> **핵심 통찰 — "수행분만 정산"은 이미 구조적(7.9 "수가 미발생"의 쌍대):** `fee_items`는 perform 시점에만 발생(진찰료=`registered→in_progress`·검사처치료=`ordered→performed`) → 미수행(`ordered`) 오더는 fee_item 0 → `build_payment`가 자동 제외 → 수행분만 청구(FR-119). 신규 정산 경로 0(기존 7.4 `finalize_payment` 가 부분수행을 정산). ⇒ 7.10 능동 작업 = **게이트(정산액 사후 변조 방지)** + 카운트 가시성.

| 용어 | 종류 | 정의 / 위치 |
|---|---|---|
| `assert_encounter_orderable(uuid)` | 함수(0053·SECURITY DEFINER) | 오더-by-내원상태 게이트 가드 — 미존재 PT404·`is_active=false`(soft-deleted) PT409·`status in (completed,cancelled,no_show)`(종결) PT409. scheduled/registered/in_progress(active)=통과. 생성 트리거·perform RPC 공용. `revoke from public,anon,authenticated`+`grant execute to service_role`(owner 컨텍스트 호출만) |
| `enforce_encounter_orderable_on_insert()` + `trg_{examinations,treatment_orders,prescriptions}_encounter_gate` | 트리거 함수·트리거(0053·SECURITY DEFINER) | 오더 생성 게이트 — 세 오더 테이블 BEFORE INSERT 가 `assert_encounter_orderable(new.encounter_id)` 호출 → 종결 내원 신규 오더 INSERT 차단(직접 API 포함·db.py insert_* 는 직접 INSERT라 DB 트리거가 권위). 기존 전이 트리거(`enforce_act_order_transition`·`enforce_prescription_transition`)와 공존 |
| `pending_orders_count` | 파생 필드(API 헤더 SELECT·비저장) | 미수행 오더 수 = Σ count(examinations·treatment_orders where status='ordered' and is_active)·내원별. `_PAYMENT_HEADER_SELECT` 상관 서브쿼리(`payments.encounter_id`·patient_name 패턴 미러). `PaymentResponse.pending_orders_count`(web `Payment` 거울). **prescriptions 제외**(issued/dispensed·수행 개념 없음·fee 0). payment.read 경로(order.read grant 0·최소권한). 부분수행 배지(billing-detail draft·청구 제외 안내) |

> ⚠️ **게이트 의미 = 종결(completed/cancelled/no_show)·soft-deleted 차단 (NOT 비-in_progress):** 방사선·간호·판독 워크리스트(`fetch_radiology/nursing/reading_worklist`)가 `e.status in ('registered','in_progress')`를 노출 → **registered 내원에 오더 생성·수행은 실제 임상 플로우**(촬영·검체·처치는 진찰 전/중). in_progress-only 게이트는 이 실기능을 깨뜨림 → 게이트는 **종결 내원(정산 후·수가 변조 위험)** 만 차단해 deferred(완료/취소 내원)를 정확히 청산하고 registered 플로우를 보존.

> **부분 수행 정산 경계(Story 7.10 확정):** **신규 마이그 1건(0053)·신규 권한 0·신규 테이블 0·신규 컬럼 0·신규 엔드포인트 0·신규 RPC 1개(가드)**. 청산 deferred-work: L277(`insert_prescription`)·L290(`insert_examination`)+treatment 생성=생성 트리거·L333(`call_perform_treatment_order`)=perform 게이트·L346(종결 내원 perform 시 수가 적재)=perform 게이트 상류 차단. API: `_PAYMENT_HEADER_SELECT` pending_orders_count 서브쿼리 + `PaymentResponse.pending_orders_count`(정산은 기존 7.4 finalize). Web: billing-detail 부분수행 배지(draft·`pending_orders_count>0`·⚠ 색비의존). **경계(하지 않음)**: 미수행 오더 자동 cancel(forward-only 유지)·`complete_examination` 게이트(종결 후 판독 정당·fee 0)·fee 트리거(0021) 변경(perform 게이트 상류 차단으로 충분)·`nursing_records` 게이트(간호 도메인·비-billable·잔여)·dispense/export 게이트(7.7·종결 후 정당). **미구현(후속)**: 환자 포털 수납=8.3.

## 환자 포털 — 수납 · 영수증 (Story 8.3, 마이그 0)

> ⚠️ **마이그레이션 0건**(7.6 선례): 8.1(0054 diagnoses.patient_friendly_note)·8.2(0055 examinations.patient_result_*)와 달리 수납 데이터(금액·`category` 한국어·`payment_method`·`clinic_profile`)가 **이미 환자 가독** → 쉬운말 큐레이션 컬럼 불요. 기존 0045~0049 스키마 + `payments_select_self`/`payment_details_select_self` RLS(0045·직접조회 심층방어) 재사용. **신규 컬럼·RPC·권한·감사 0**. **설계 결정(사용자 확정·AskUserQuestion 3건)**: ① 화면=**'마이' 탭(`/portal`)** 수납 리스트(placeholder 대체·mockup IA) ② 영수증=**친화 요약(화면·쉬운 말) + "영수증 인쇄"로 7.5 `ReceiptDocument`(Batang serif 법정 서식) 재사용**(`hidden print:block`·@media print) ③ 조회 범위=**finalized 만**(draft·cancelled 제외).

| 용어 | 종류 | 정의 / 위치 |
|---|---|---|
| `PatientPaymentCard` | 스키마(patients.py)·web 거울 | 환자 포털 '마이' 탭 finalized 수납 카드 — `encounter_id`(영수증 라우팅 키)·`payment_no`·`clinic_name`·`department_name`·`treatment_date`·`finalized_at`·`total_amount_krw`·`paid_amount_krw`(내가 낸 금액)·`payment_method`·`status`. ⚠️ raw RRN/연락처/`*_by` 미투영(비-PII) |
| `fetch_self_payments`/`list_self_payments` | db·service(8.3) | 본인 finalized 수납 카드 목록 — `where p.auth_uid=$1 and pay.status='finalized'`(세션 uid 스코프·service_role RLS 우회 경계선)·최근순·clinic_name 스칼라 서브쿼리. 미연결=빈 목록(8.1 패턴) |
| `fetch_self_receipt`/`get_self_receipt` | db·service(8.3) | 본인 영수증 데이터 — 7.5 `fetch_receipt` 를 self-scope 로 미러(`_SELF_RECEIPT_HEADER_SELECT`=`_RECEIPT_HEADER_SELECT` + `and pat.auth_uid=$2`·조립은 `_assemble_receipt_payload` 공용). 소유 미일치·**비-finalized → None → 404**(직원 7.5 는 409·환자 self 는 draft 존재 비노출·IDOR 일관). 7.5 `ReceiptResponse` 재사용(신규 스키마 0) |
| `GET /patients/me/payments` · `GET /patients/me/encounters/{id}/receipt` | 라우트(8.3·get_current_patient 게이트·직원 403) | self 수납 리스트·영수증. patient_id 미수용(서버 auth_uid=sub 도출)·`/me/*` 정적 프리픽스(8.1/8.2 순서 선례). 직원용 7.5 `/encounters/{id}/payment/receipt`(payment.read)와 별도 self 경로(권한 아닌 소유 게이트) |
| `payment-history`·`receipt-detail`·`lib/patient/payments` | web(8.3) | 마이 탭 수납 리스트(`/portal`·신뢰 노트·완료 배지·`/receipts/{id}` 링크)·영수증 상세(`/receipts/[encounterId]`·친화 요약 + `hidden print:block` ReceiptDocument 인쇄)·`fetchSelfPayments`/`fetchSelfReceipt`/`aggregateAmountByCategory`(표시 그룹핑). 인쇄 파일명 PII 금지(`document.title=영수증_{chart_no}` 불투명). **환자 본인 열람·인쇄=감사 미적재**(self-read 무감사·UX-DR22 내보내기 감사는 직원 타인 문서 정책) |

## 운영 대시보드 — 통계 (Story 8.5, 마이그 0)

> ⚠️ **마이그레이션 0건**: 집계 데이터 소스(`encounters`·`payments`·`appointments`·`payment_details`)가 전부 기존(0010/0045/0031/0052). **신규 테이블·컬럼·권한·RPC·트리거 0**(`dashboard.read`=0002 기존·admin boot grant). 복잡 집계는 **FastAPI 서비스 계층 SQL**이 담당(architecture L193·"복잡 집계→FastAPI") — DB 함수로 빼지 않는다(명세 실증). read-only(쓰기/상태전이 0). **설계 결정**: ① 경로=`/admin/dashboard`(staff-nav 기존 항목·`dashboard.read`) ② 차트=신규 의존성 없이 인라인 막대(값·일자 라벨 병기·색/음영 비의존·A3) ③ 매출=환자 순수납액 Σ(paid−refunded)(8.5=`refunded_amount_krw` 첫 리포팅 소비처) ④ 추세 윈도우 14일 기본.

| 용어 | 종류 | 정의 / 위치 |
|---|---|---|
| `fetch_dashboard_operations(sub, *, on_date, days)` | 함수(db·8.5) | 운영 대시보드 집계 — 당일 스냅샷(encounters 상태 카운트) + 윈도우 [on_date−(days−1), on_date] 일별 시리즈(내원·순수납액·노쇼). `_run_authed`/`authenticated_conn`(service_role·RLS 우회·권위=라우터 `dashboard.read`·billing worklist 동일 방어심층). 모든 일자 그룹 = `(col at time zone 'Asia/Seoul')::date`(UTC→KST). 빈 날=0(누락 일자 Python 채움) |
| 일별 메트릭(KST) | 집계 규칙(8.5) | **내원**=`registered_at` KST·status in (registered/in_progress/completed)(취소·노쇼·scheduled 제외) · **대기/진료중/완료**=당일 내원분(`registered_at` KST) 중 현재 status registered/in_progress/completed(**세 카운트 모두 registered_at 코호트 → 대기+진료중+완료=내원 불변식**·자정 넘김 "완료>내원" 모순 회피) · **순수납액**=Σ(`paid_amount_krw`−`coalesce(refunded_amount_krw,0)`)·finalized·`finalized_at` KST · **노쇼율**=`appointments.status='no_show'` / (no_show+completed)·`scheduled_start` KST(cancelled·booked 제외·분모 0→0.0) |
| `DashboardOperationsResponse`·`DashboardTodaySnapshot`·`DashboardDailyPoint` | 스키마(dashboard.py)·web 거울 | 단일 리소스 응답(목록 아님→`{data,meta}` 미사용)·`as_of_date`+`today`+`daily_series`(오래된→최신). 금액 KRW 정수·비율 0~1 float·날짜 ISO·전 필드 snake_case |
| `get_operations(sub, *, on_date, days)` | 함수(service·8.5) | db 집계 호출 + Pydantic 매핑. `on_date` 미지정→**KST 오늘**(서버 결정·클라 신뢰 X). days 1~90(라우터 가드·기본 14) |
| `GET /v1/dashboard/operations?date&days` | 라우트(8.5·`require_permission("dashboard.read")`) | 운영 통계 조회(FR-230). 미인증 401·무권한 403(`required_permission: dashboard.read`). clinic-wide 집계(관리자 범위)·PII 미포함 |
| `operations-dashboard`·`lib/dashboard/operations`·`/admin/dashboard` | web(8.5) | 대시보드 페이지(서버 가드 `requirePermission("dashboard.read")`)+클라 컴포넌트(KPI 스탯 카드 6 + 인라인 추세 막대 3·`useState` 단일 로드·billing-worklist 패턴). `fetchDashboardOperations`/`formatPercent`/`compactKrw`/`monthDayLabel`. nav 노출=admin+`dashboard.read`(staff-nav 기존). 차트=신규 라이브러리 0(인라인 SVG/div·A3 색비의존) |

> **운영 대시보드 경계(Story 8.5 확정):** **신규 마이그 0·신규 권한 0·신규 테이블/컬럼 0**(전부 누적 데이터 위 read-only 집계). API: `fetch_dashboard_operations`(db)·`get_operations`(service)·`dashboard.py` 라우터(router.py 등록)·`schemas/dashboard.py`. Web: `lib/dashboard/operations.ts`·`/admin/dashboard/page.tsx`·`components/dashboard/operations-dashboard.tsx`. demo_seed 보강: 완료 내원 e01~e06 에 finalized 수납 직접 적재(매출 추세 가시성·payments cleanup 추가·payment_no=R-YYYYMMDD-sfx 비충돌). **단순화/이월**: 대기/진료중=now 스냅샷(과거일 질의 시 0 근사)·매출=환자 순수납액만(공단부담 분해 미표시)·환급은 원 finalize 일자 귀속(데모 단순화)·진료과별/의사별 드릴다운·기간 비교·CSV 내보내기=후속.
