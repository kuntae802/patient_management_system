...
| `payment` | 수납 | 헤더 — `payments`(0045·7.1)·내원 1:1(encounter_id UNIQUE)·status draft/finalized/cancelled·billing_type postpaid/prepaid·금액 집계 컬럼은 7.2 집계(`build_payment` 0046·total/covered/non_covered)/7.3 산정/7.4 결제(`finalize_payment` 0048·payment_method·payment_no·finalized_at/by·paid)가 채움. **문서 출력=7.5/7.6**(진료비 계산서·영수증 `0049`·**세부산정내역서 7.6**·브라우저 인쇄·masked RRN·내보내기 감사 `log_payment_document_export`·document_type=receipt/statement) |
