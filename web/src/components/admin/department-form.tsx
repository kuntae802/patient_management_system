...
import {
  type Department,
  type DepartmentCreateValues,
  departmentCreateSchema,
  toDepartmentCreatePayload,
  toDepartmentUpdatePayload,
...
// 진료과 생성/수정 폼(모달). RHF + Zod(Pydantic 거울). 쓰기 = FastAPI(apiFetch, master.manage).
// editing 지정 시 수정(PATCH, code 불변·읽기전용), 미지정 시 생성(POST). 변경은 0006 자동 감사.
export function DepartmentForm({
  ...
  // editing 이 바뀌면 폼 값을 동기화(RHF values). 생성 모드는 빈 기본값.
  const values: DepartmentCreateValues = useMemo(
    () => ({
      code: editing?.code ?? "",
      name: editing?.name ?? "",
  ...
  } = useForm<DepartmentCreateValues>({
    resolver: zodResolver(departmentCreateSchema),
    values,
  });

  async function onSubmit(formValues: DepartmentCreateValues) {
    try {
      const saved = editing
        ? await apiFetch<Department>(`/v1/masters/departments/${editing.id}`, {
            method: "PATCH",
            body: JSON.stringify(toDepartmentUpdatePayload(formValues)),
          })
        : await apiFetch<Department>("/v1/masters/departments", {
            method: "POST",
            body: JSON.stringify(toDepartmentCreatePayload(formValues)),
          });
      onSaved(saved);
      toast.success(`${saved.name} 진료과가 ${editing ? "수정" : "생성"}되었습니다.`);
      onOpenChange(false);
    } catch (err) {
      const message =
        err instanceof ApiError ? err.message : "진료과를 저장하지 못했습니다. 다시 시도해 주세요.";
      ...
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100vh-2rem)] w-[min(480px,calc(100vw-2rem))] -translate-x-1/2 -translate-y-1/2 overflow-auto rounded-xl border border-border bg-card p-5 outline-none">
          <Dialog.Title className="text-[15px] font-semibold text-foreground">
            {editing ? "진료과 수정" : "진료과 추가"}
          </Dialog.Title>
          <Dialog.Description className="mt-1 text-[12.5px] text-muted-foreground">
            {editing
              ? "코드는 변경할 수 없습니다. 이름·설명을 수정합니다."
              : "예약·접수·근무표가 참조할 조직 단위를 등록합니다."}
          </Dialog.Description>
