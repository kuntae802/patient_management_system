...
export function TreatmentPanel({
  ...
  async function cancel(orderId: string) {
    if (busy) return;
    if (!window.confirm("처치 오더를 취소하시겠습니까?")) return;
    setBusy(true);
    try {
      await cancelTreatmentOrder(encounterId, orderId);
      await onReload();
      toast.success("처치 오더를 취소했습니다.");
    } catch (err) {
      toast.error(
        err instanceof ApiError ? err.message : "처치 취소에 실패했습니다.",
      );
            ...
            {ordered.map((tr) => (
              <li
                ...
                className={`rounded-md border border-border bg-card px-2.5 py-2${
                  tr.status === "cancelled" ? " opacity-60" : ""
                }`}
              >
