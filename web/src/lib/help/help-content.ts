...
export const HELP_GUIDES: Record<string, HelpMenuGuide> = {
  ...
  // ── 관리자(admin) 메뉴 (Story 9.7) — 운영/대시보드·마스터·권한·근무 스케줄·직원 계정·감사 로그 ──
  "/admin/dashboard": {
    href: "/admin/dashboard",
    intro:
      "운영 현황을 한눈에 봅니다. 조회 기준일과 집계 기간을 골라 선택일 스냅샷·기간 합계(월간 수치)·일별 추세를 확인합니다. 읽기 전용이라 이 화면에서 바뀌는 데이터는 없습니다.",
    screens: [
      {
        title: "운영 대시보드",
        image: "/help/admin/dashboard.png",
        imageWidth: 1440,
        imageHeight: 900,
        alt: "관리자 운영 대시보드 — 조회 컨트롤·현황 카드·일별 추세 차트",
        hotspots: [
          { num: 1, element: "조회 기준일", desc: "현황을 볼 날짜를 고릅니다. 비우면 오늘 기준입니다." },
          { num: 2, element: "집계 기간", desc: "합계·추세를 집계할 기간(최근 7·14·30·90일)을 고릅니다. 30일이면 한 달치, 더 짧게 잡으면 일별에 가까운 추이를 봅니다." },
          { num: 3, element: "최근 N일 합계", desc: "선택한 기간의 내원·순수납액·노쇼 누계입니다(월간 수치)." },
          { num: 4, element: "선택일 현황", desc: "조회 기준일 하루의 내원·대기·진료중·완료·순수납액·노쇼율 스냅샷입니다." },
          { num: 5, element: "일별 추세", desc: "일별 내원·순수납액·노쇼율을 막대로 보여줍니다. 막대 위 숫자와 아래 날짜로 추이를 읽습니다." },
        ],
        flow: "기준일과 집계 기간을 바꿔 월간·일별 추세와 기간 합계를 확인합니다. 모두 읽기 전용 — 집계·표시만 합니다.",
      },
    ],
  },
  "/admin/masters": {
    href: "/admin/masters",
    intro:
      "진료과·진료실과 진단·수가·약품 코드 마스터를 관리합니다. 비활성하거나 만료된 항목은 신규 선택에서 빠지되 과거 기록의 참조는 보존되고, 모든 변경은 감사 로그에 남습니다.",
    screens: [
      {
        title: "마스터",
        image: "/help/admin/masters.png",
        imageWidth: 1440,
        imageHeight: 900,
        alt: "관리자 마스터 — 5종 탭과 진료과 목록",
        hotspots: [
          { num: 1, element: "마스터 탭", desc: "진료과·진료실·진단·수가·약품 5종을 탭으로 전환합니다. 탭 옆 숫자는 등록 건수입니다." },
          { num: 2, element: "추가", desc: "현재 탭에 새 항목을 등록합니다(탭에 따라 라벨이 “진료과 추가”·“수가 추가” 등으로 바뀝니다)." },
          { num: 3, element: "코드", desc: "각 항목의 코드입니다(옆에 이름·설명이 이어집니다). 코드 순으로 정렬됩니다." },
          { num: 4, element: "상태", desc: "활성·비활성(코드 마스터는 발효/만료) 상태입니다. 비활성·만료는 신규 선택에서 제외됩니다." },
          { num: 5, element: "수정·비활성", desc: "항목을 수정하거나 활성/비활성을 전환합니다. 비활성은 확인을 거칩니다." },
        ],
        flow: "탭으로 5종 마스터를 오가며 등록·수정합니다. 비활성/만료 항목은 새 선택에서 빠지고 과거 기록은 그대로 보존되며, 변경은 감사 로그에 기록됩니다.",
      },
    ],
  },
  "/admin/permissions": {
    ...
    intro:
      "역할별로 어떤 기능에 접근할 수 있는지 격자(역할 × 권한)로 관리합니다. 셀을 누르면 즉시 적용·자동 저장되고 감사 로그에 남습니다 — 별도 저장 버튼이 없습니다.",
    screens: [
      {
        title: "권한 관리",
        image: "/help/admin/permissions.png",
        imageWidth: 1440,
        imageHeight: 900,
        alt: "관리자 권한 매트릭스 — 역할 × 권한 토글 격자",
        hotspots: [
          { num: 1, element: "즉시 적용 안내", desc: "변경은 즉시 적용되고 감사 로그에 기록됩니다. 저장 버튼 없이 셀 변경만으로 반영됩니다." },
          { num: 2, element: "범례(고정·민감)", desc: "허용·차단·고정(관리자)·민감을 표시합니다. 관리자 역할은 항상 전체 허용이라 바꿀 수 없습니다(고정)." },
          { num: 3, element: "역할 열", desc: "원무과·의사·간호사·방사선사·관리자 역할입니다. 열 아래 숫자는 그 역할의 허용 권한 수입니다." },
          { num: 4, element: "권한 행", desc: "기능 권한입니다. ‘민감’ 표시(주민번호 열람 등)는 변경 시 한 번 더 확인합니다." },
          { num: 5, element: "허용/차단 셀", desc: "셀을 눌러 그 역할의 권한을 켜고 끕니다. 즉시 반영되며, 관리자 열은 고정되어 누를 수 없습니다." },
        ],
        flow: "역할(열)과 기능 권한(행)이 만나는 셀을 눌러 접근을 켜고 끕니다. 변경은 즉시 적용·자동 저장되고, 민감 권한은 확인을 거치며, 관리자 역할은 안전을 위해 고정됩니다.",
      },
    ],
  },
  "/admin/schedule": {
    href: "/admin/schedule",
    intro:
      "의사 근무표와 휴진·예외를 관리합니다. 근무표는 예약 가능 슬롯의 근거가 되고, 휴진은 그 기간 예약의 재배정 근거가 됩니다. 비활성해도 과거 예약 참조는 보존되며 변경은 감사 로그에 남습니다.",
    screens: [
      {
        title: "근무 스케줄",
        image: "/help/admin/schedule.png",
        imageWidth: 1440,
        imageHeight: 900,
        alt: "관리자 근무 스케줄 — 근무표 탭과 의사 주간 근무 목록",
        hotspots: [
          { num: 1, element: "근무표·휴진 탭", desc: "근무표와 휴진·예외를 탭으로 전환합니다. 탭 옆 숫자는 등록 건수입니다." },
          { num: 2, element: "근무표 추가", desc: "새 근무(의사·요일·시간대·진료실)를 등록합니다(휴진 탭에서는 ‘휴진·예외 등록’)." },
          { num: 3, element: "의사", desc: "근무하는 의사입니다(옆에 진료과·요일이 이어집니다)." },
          { num: 4, element: "시간대", desc: "근무 시간대입니다. 예약 가능 슬롯이 여기서 계산됩니다." },
          { num: 5, element: "상태", desc: "활성/비활성 상태입니다. 옆 ‘관리’에서 수정·전환하고, 휴진 탭에서는 ‘영향 예약’으로 재배정합니다." },
        ],
        flow: "근무표 탭에서 의사별 요일·시간대 근무를 등록하면 예약 슬롯의 근거가 됩니다. 휴진·예외 탭에서 휴진을 등록하면 그 기간 예약이 ‘영향 예약’으로 표시되어 재배정할 수 있고, 과거 예약 참조는 보존됩니다.",
      },
    ],
  },
  "/admin/users": {
    href: "/admin/users",
    intro:
      "직원 계정을 만들고 역할·소속 진료과·재직상태를 관리합니다. 모든 변경은 감사 로그에 남습니다.",
    screens: [
      {
        title: "직원 계정",
        image: "/help/admin/users.png",
        imageWidth: 1440,
        imageHeight: 900,
        alt: "관리자 직원 계정 — 직원 목록과 역할·재직상태",
        hotspots: [
          { num: 1, element: "계정 추가", desc: "새 직원 계정을 만듭니다(사번·이름·역할·면허 등)." },
          { num: 2, element: "새로고침", desc: "직원 목록을 다시 불러옵니다." },
          { num: 3, element: "사번·이름·역할", desc: "직원의 사번·이름·역할과 면허입니다." },
          { num: 4, element: "소속 진료과", desc: "직원의 소속 진료과를 배정·변경합니다." },
          { num: 5, element: "재직상태", desc: "재직·휴직·퇴사를 전환합니다. 휴직·퇴사는 확인을 거칩니다." },
        ],
        flow: "‘계정 추가’로 직원을 등록하고 역할·소속 진료과·면허를 지정합니다. 재직상태를 바꾸면(휴직·퇴사는 확인) 즉시 반영되고, 모든 변경은 감사 로그에 남습니다.",
      },
    ],
  },
  "/admin/audit-logs": {
    ...
    intro:
      "누가 언제 무엇을 했는지 감사 로그를 조회합니다. 읽기 전용이며 편집·삭제가 불가능합니다(append-only). 환자·보호자 이름은 마스킹됩니다.",
    screens: [
      {
        title: "감사 로그",
        image: "/help/admin/audit-logs.png",
        imageWidth: 1440,
        imageHeight: 900,
        alt: "관리자 감사 로그 — 필터와 행위자·동작·대상 목록",
        hotspots: [
          { num: 1, element: "동작 필터", desc: "생성·수정·열람 등 동작으로 거릅니다." },
          { num: 2, element: "대상 필터", desc: "대상 종류(내원·수납·간호기록 등)로 거릅니다." },
          { num: 3, element: "기간(시작일)", desc: "시작일·종료일로 기간을 좁힙니다(옆에 행위자 필터도 있습니다)." },
          { num: 4, element: "초기화", desc: "걸어둔 필터를 모두 해제합니다." },
          { num: 5, element: "로그 목록", desc: "시각·행위자·동작·대상이 한 줄씩 쌓입니다. 환자·보호자 이름은 마스킹됩니다." },
          { num: 6, element: "보기", desc: "한 건의 상세를 읽기 전용으로 엽니다(편집·삭제 불가)." },
        ],
        flow: "동작·대상·행위자·기간으로 좁혀 추적합니다. 감사 로그는 읽기 전용(append-only)이라 편집·삭제할 수 없고, 50건씩 페이지로 넘겨 봅니다.",
      },
    ],
  },
};
